...
if (isset($_POST['actualizar'])) {
    ...
    $imagen_rel = $_POST['imagen_actual'] ?? null;
    if (!empty($_FILES['imagen']['name'])) {
        // eliminar imagen anterior
        if ($imagen_rel && file_exists(__DIR__ . DIRECTORY_SEPARATOR . $imagen_rel)) {
            @unlink(__DIR__ . DIRECTORY_SEPARATOR . $imagen_rel);
        }
        $basename = time() . "_" . preg_replace('/[^A-Za-z0-9_\-\.]/', '_', basename($_FILES["imagen"]["name"]));
        $archivo_abs = $upload_dir_abs . $basename;
        if (move_uploaded_file($_FILES["imagen"]["tmp_name"], $archivo_abs)) {
    ...
    $stmt->bind_param("sssdii", $nombre, $descripcion, $imagen_rel, $precio, $cantidad, $id);
    $stmt->execute();
    $stmt->close();
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        body{font-family: "Segoe UI", Tahoma, sans-serif;background:var(--bg);color:#222;display:flex;height:100vh;}
        header{position:fixed;top:0;left:0;width:100%;height:60px;background:var(--lila);color:#fff;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:600;box-shadow:0 2px 6px rgba(0,0,0,0.15);z-index:100;}
        .sidebar{width:280px;background:#fff;padding:80px 20px 20px;border-right:1px solid #eee;overflow-y:auto;}
        .content{flex:1;padding:80px 30px 30px;overflow:auto;}
        ...
        .btn{background:var(--lila);color:#fff;padding:10px 14px;border:none;border-radius:8px;cursor:pointer;font-weight:600;text-decoration:none;display:inline-block;text-align:center;transition:0.2s;}
        .btn:hover{background:var(--lila-oscuro)}
        .btn.delete{background:var(--lila-oscuro);}
        .btn.delete:hover{background:var(--lila);}
        table{width:100%;border-collapse:collapse;background:#fff;border-radius:var(--radius);overflow:hidden;box-shadow:0 2px 6px rgba(0,0,0,0.05);}
    ...
    <aside class="sidebar">
        <?php if ($editando && $producto_editar): ?>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="imagen_actual" value="<?= htmlspecialchars($producto_editar['imagen']) ?>">
                <label>Nombre</label>
                <input type="text" name="nombre" value="<?= htmlspecialchars($producto_editar['nombre']) ?>" required>
                <label>Descripción</label>
                <textarea name="descripcion" required><?= htmlspecialchars($producto_editar['descripcion']) ?></textarea>
                <label>Imagen (opcional para cambiar)</label>
                <?php if ($producto_editar['imagen']): ?>
                    <img src="<?= htmlspecialchars($producto_editar['imagen']) ?>" class="thumb" alt="actual"><br>
                <?php endif; ?>
                <input type="file" name="imagen" accept="image/*">
                <label>Precio</label>
                <input type="number" step="0.01" name="precio" value="<?= $producto_editar['precio'] ?>" required>
                <label>Cantidad</label>
                <input type="number" name="cantidad" value="<?= $producto_editar['cantidad'] ?>" required>
                <button class="btn" type="submit" name="actualizar" style="width:100%;margin-top:10px">Actualizar</button>
                <a class="btn delete" href="<?= $_SERVER['PHP_SELF'] ?>" style="width:100%;margin-top:10px">Cancelar</a>
            </form>
        <?php else: ?>
            ...
            <table>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <!--  Aquí se aplica formato de moneda -->
                        <td><?= "$" . number_format($row['precio'], 0, ',', '.') ?></td>
                        <td><?= htmlspecialchars($row['cantidad']) ?></td>
                        <td><?= htmlspecialchars($row['fecha_creacion']) ?></td>
                        <td><?= htmlspecialchars($row['fecha_actualizacion']) ?></td>
                        <td>
                            <a class="btn" href="?editar=<?= intval($row['id']) ?>">Editar</a>
